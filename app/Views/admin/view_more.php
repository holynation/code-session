...
<div class="container-xxl flex-grow-1">
	...
	?>
                  <div class="row d-flex">
                    <div class="<?=$colClass;?>">
                        <div class="card">
                          <h5 class="card-header mb-3"><?php echo ucwords($pageTitle); ?></h5>
...
$entityID = @$extra[4];
	...
	?>
                      ...
                      <div class="my-4 ml-4">
                        ...
                        <!-- this is for fund rollover modal form -->
                        <div id="rollover-modal-fund" class="modal fade" role="dialog" tabindex="-1" aria-hidden="true">
                          <div class="modal-dialog modal-dialog-top" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                  <h5 class="modal-title">Fund <i><?php echo $modelPayload['fullname']; ?></i> Rollover Wallet</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                      <span aria-hidden="true">&times;</span></button>
                                </div>
                                <div class="modal-body">
                                    ...
                                    <form action="" method="post" role="form" id="form_rollover_wallet" name="form_rollover_wallet" onsubmit='updateAccountRolloverWallet("<?=$user_id?>","rollover-fund",$("#fund_rollover_amount").val(),"<?=$entityType?>","<?=$updateType?>");return false;'>
                                      <div class="form-group mb-3">
                                          <label for="amount">Enter the Amount</label>
                                          <input class="form-control" type="number" required name="fund_rollover_amount" id="fund_rollover_amount" placeholder="Actual amount customer paid into wallet">
                      ...
                      <div class="my-4 ml-4">
                        <?php if ($entityType != 'withdrawal'): ?>
                        <button class="btn btn-primary mb-1" data-toggle='modal' data-target='#rollover-modal-fund'>First Rollover Funding</button>
                        <?php endif;?>
                      </div>
...
<script type="text/javascript">
  ...
  function updateAccountRolloverWallet(userID,type,amount,pageType,pageVal){
    const link = "<?php echo base_url('ajaxData/updateAccountRolloverWallet/'); ?>" +'/'+ type;
    if (confirm("Are you sure you want to "+type+" item?")) {
      var data = new FormData();
      data.append('user_id',userID);
      data.append('amount',amount);
